...
public class BatalhaNaval {
	...
	private String[] orientacaoValidada = {"V","v"};	
	private int[] frotaValida = {1,2,3};
	private String[] linhaValida = {"A","a","B","b","C","c","D","d","E","e","F","f"};
	private int[] colunaValida = {1,2,3,4,5,6};
	...
	public BatalhaNaval( int t ) throws Error {
		if(t>10){
			throw new Error("ExcBN01. Tamanho da matriz não pode ser maior que 10");
		}else if(t<6){
			throw new Error("ExcBN01. Tamanho da matriz não pode menor que 6");
		}
		tamanho = t;
	...
	public void imprimirTabuleiros(){
		String jogadores = "";
		String colunas = "";
	...
	public boolean validaEntradaDados(String entrada) throws Exception{
		if(entrada.isEmpty() || (entrada.length() != 6 && entrada.length() != 1) ){
			throw new Exception("Comando inválido, siga as instruções de preenchimento. ex: \"1 B2 H\" ou \"I\"");
		}
			...
			if(input.length() == 1){
				if(input.charAt(0) == 'I'){
					if(battleship.placed && destroyer.placed && sub.placed)
						...
						System.out.println("Distribua toda a frota no Campo de Batalha.");
				} else {
				...
				else if(((int)input.charAt(3)-48) > 6){
				...
				System.out.println("Texto inválido, siga as instruções de preenchimento. ex: \"1 B2 H\" ou \"I\"");
			}
		}
	}
